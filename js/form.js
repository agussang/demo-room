...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // CHSE Verification
  document
    .getElementById("check-chse")
    .addEventListener("click", function () {
      const chseInput = document.getElementById("chse");
      if (chseInput.value.trim() !== "") {
        this.innerHTML =
          '<i class="fas fa-check-circle"></i> Terverifikasi';
        this.classList.remove("btn-outline-primary");
        this.classList.add("btn-success");
        ...
        // Update status card
        document.getElementById("status-chse").textContent =
          "Terverifikasi";
        document.getElementById("status-chse").className = "text-success";

        // Show notification
        showNotification(
          "CHSE Terverifikasi",
          "Sertifikat valid dan terverifikasi melalui API CHSE",
          "success"
        );
      } else {
        showNotification(
          "Verifikasi Gagal",
          "Silakan masukkan nomor CHSE terlebih dahulu",
          "error"
        );
      }
    });

  // Update hotel name status when input changes
  document
  ...
  // Update classification status when select changes
  document
